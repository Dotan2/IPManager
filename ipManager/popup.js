...
class IPManager {
  ...
  setupEventListeners() {
    ...
    // Add environment modal controls
    document.getElementById('closeAddEnvModal').addEventListener('click', () => {
      this.hideModal('addEnvModal');
    });

    document.getElementById('cancelAddEnv').addEventListener('click', () => {
      this.hideModal('addEnvModal');
    });

    // Add environment form submission
    document.getElementById('addEnvForm').addEventListener('submit', (e) => {
      e.preventDefault();
      this.saveNewEnvironment();
    });
  ...
  saveNewEnvironment() {
    ...
    if (envName) {
      ...
      this.showFeedback(`Added environment "${envName}"`);
    }
  }
  ...
  updateDeleteButtonVisibility() {
    ...
    // Show delete button only if a specific environment is selected (not "All Environments")
    // and it's not the default environment
  ...
  deleteCurrentEnvironment() {
    ...
    const env = this.data.envs.find(e => e.id === selectedEnv);
    if (!env) {
      return;
    }
    ...
    const confirmed = confirm(
      ...
      `This will also delete all ${env.items.length} hosts in this environment.\n\n` +
      `This action cannot be undone.`
    );

    if (confirmed) {
      // Move all items from this environment to the default environment
      const defaultEnv = this.data.envs.find(e => e.id === 'default');
      if (defaultEnv) {
        env.items.forEach(item => {
          item.environment = 'default';
          defaultEnv.items.push(item);
        });
      }

      // Remove the environment
      const envIndex = this.data.envs.findIndex(e => e.id === selectedEnv);
      if (envIndex !== -1) {
        this.data.envs.splice(envIndex, 1);
      }
      ...
      this.filterItems();
      this.renderHostList();
      this.showFeedback(`Deleted environment "${env.name}" and moved ${env.items.length} hosts to Default`);
    }
  }
